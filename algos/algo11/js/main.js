...
function LRJF(mode , proc){
  if (pret.processors.length != 0 || blocked.processors.length != 0 || processor.inProcess.length != 0){
        ...
        if (elem.pere == -1 ){
            sleep(SPEED + elem.left_time * TIME_UNIT).then(() => {
              ...
              sleep(SPEED + elem.left_time * TIME_UNIT).then(() => {
                  ...
                  push_history_inProcess_pret(elem, elem.left_time) // CORRECT
                  finish_process();elem.pere.treat_int();resume_process(elem.pere);sleep(SPEED).then(() => {LRJF()})})
        }
        }else{
    ...
    if (mode == "block"){
      ...
      clean_data(ALLL)
      sleep(SPEED).then(() => { alert("Simualation LRJF have finished")})
      sleep(2000).then(() => {
        document.getElementById('gantt_div').style.display = "block" ;
        document.getElementById('tab_div').style.display = "block" ;
          draw_gantt_(data_LRJF, "FCFS_");
          plot_time_table(all_histories["LRJF"], "plot_time")
