...
function update_fifos() {
    ...
    for (var j = 0; j < q_l.length; j++) {
        if (!isNaN(parseInt(q_l[j]))) {
            q_l_.push(parseInt(q_l[j]))
        }
    }

    q_l_ = q_l_.sort(function (a, b) {
        return a - b;
    })
    QUANTUMS = q_l_
    ...
    var NB_FIFO = [4, q_l_.length].sort(function (a, b) {
        return a - b;
    })[0];
    list_fifos = [];
...
function MULTI_NV(mode, proc) {
    if (list_fifos_not_empty() || blocked.processors.length != 0 || processor.inProcess.length != 0) {
        if (processor.isready() && list_fifos_not_empty() != 0) {
            ...
            if (!elem.hasint()) { //If processor doesn't have int
                if (elem.left_time <= qntm) {
                    ...
                    sleep(SPEED).then(() => {
                        update_left_time(elem, elem.left_time, elem.left_time - qntm, 1);
                    })
        ...
        sleep(SPEED).then(() => {
            alert("Simulation Round Robin have finished")
        })
        sleep(2000).then(() => {
            ...
            plot_time_table(all_histories["MULTI_NV"], "plot_time")
