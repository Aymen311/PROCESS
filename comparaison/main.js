...
function get_cpu_usage() {
    ...
    for (var p = 0; p < ALLL.length; p++) {
        if (max < ALLL[p].history[[ALLL[p].history.length - 1]][0]) {
            max = ALLL[p].history[[ALLL[p].history.length - 1]][0]
        }
    }
        ...
        for (var p = 0; p < ALLL.length; p++) {
            for (var state = 0; state < ALLL[p].history.length; state++) {
                if ((ALLL[p].history[state][0] <= i) && (i <= ALLL[p].history[state][1])) {
                    if (ALLL[p].history[state][2] != "Blocked") {
                        b = 0
                    }
                    break;
                }
...
function Doughnut(D, labels, elem) {
    var ctx = document.getElementById(elem);
    ctx.height = 400
    ctx.width = 600
